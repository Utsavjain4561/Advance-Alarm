...
public class MathMethod extends Fragment {
    ...
    public void onActivityCreated(@Nullable Bundle savedInstanceState) {
        ...
        int id=args.getInt(BaseActivity.ALARM_ID);
    ...
    private int setProblem(){
        ...
        String prob="";
            ...
            int fNum=(int)(Math.random()*(thresh-1)+1);
            prob+=fNum;
            int operatorId=3;
            prob+=opId(operatorId);
            int sNum=(int)(Math.random()*(thresh-1)+1);
            prob+=sNum;
            ans=perform(fNum,3,sNum);

        for(int i=1;i<opTh-1;++i){
            int opeatorId1=(int)(Math.random()*(opTh-1)+1);
